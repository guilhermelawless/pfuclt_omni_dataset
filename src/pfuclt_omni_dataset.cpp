...
void SelfRobot::initPFset()
{
  ...
  /*
//     ippsRandUniform_Direct_32f (&particleSet_[0][0], nParticles_,
fullStateOMNI4.robotPose[0].X-0.5, fullStateOMNI4.robotPose[0].X+0.5, &seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[1][0], nParticles_,
fullStateOMNI4.robotPose[0].Y-0.5, fullStateOMNI4.robotPose[0].Y+0.5, &seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[2][0], nParticles_,
fullStateOMNI4.robotPose[0].Theta-0.001,
fullStateOMNI4.robotPose[0].Theta+0.001, &seed_);
//
//     ippsRandUniform_Direct_32f (&particleSet_[3][0], nParticles_,
fullStateOMNI3.robotPose[0].X-0.5, fullStateOMNI3.robotPose[0].X+0.5, &seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[4][0], nParticles_,
fullStateOMNI3.robotPose[0].Y-0.5, fullStateOMNI3.robotPose[0].Y+0.5, &seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[5][0], nParticles_,
fullStateOMNI3.robotPose[0].Theta-0.001,
fullStateOMNI3.robotPose[0].Theta+0.001, &seed_);


    ippsRandUniform_Direct_32f (&particleSet_[0][0], nParticles_, 0, 6.0,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[1][0], nParticles_, -4.5, 4.5,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[2][0], nParticles_, -PI, PI,
&seed_);

    ippsRandUniform_Direct_32f (&particleSet_[3][0], nParticles_, 0, 6.0,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[4][0], nParticles_, -4.5, 4.5,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[5][0], nParticles_, -PI, PI,
&seed_);

//     ippsRandUniform_Direct_32f (&particleSet_[6][0], nParticles_,
fullStateOMNI1.robotPose[0].X-0.5, fullStateOMNI1.robotPose[0].X+0.5, &seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[7][0], nParticles_,
fullStateOMNI1.robotPose[0].Y-0.5, fullStateOMNI1.robotPose[0].Y+0.5, &seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[8][0], nParticles_,
fullStateOMNI1.robotPose[0].Theta-0.001,
fullStateOMNI1.robotPose[0].Theta+0.001, &seed_);

    ippsRandUniform_Direct_32f (&particleSet_[9][0], nParticles_,
initArray[6]-0.5, initArray[6]+0.5, &seed_);
    ippsRandUniform_Direct_32f (&particleSet_[10][0], nParticles_,
initArray[7]-0.5, initArray[7]+0.5, &seed_);
    ippsRandUniform_Direct_32f (&particleSet_[11][0], nParticles_, PI-0.001,
PI+0.001, &seed_);

    ippsRandUniform_Direct_32f (&particleSet_[6][0], nParticles_, 0, 6.0,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[7][0], nParticles_, -4.5, 4.5,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[8][0], nParticles_, -PI, PI,
&seed_);

//     ippsRandUniform_Direct_32f (&particleSet_[9][0], nParticles_, 0, 6.0,
&seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[10][0], nParticles_, -4.5, 4.5,
&seed_);
//     ippsRandUniform_Direct_32f (&particleSet_[11][0], nParticles_, -PI, PI,
&seed_);

    ippsRandUniform_Direct_32f (&particleSet_[12][0], nParticles_, 0, 6.0,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[13][0], nParticles_, -4.5, 4.5,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[14][0], nParticles_, -PI, PI,
&seed_);

    ippsRandUniform_Direct_32f (&particleSet_[15][0], nParticles_, 0, 6.0,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[16][0], nParticles_, -4.5, 4.5,
&seed_);
    ippsRandUniform_Direct_32f (&particleSet_[17][0], nParticles_, 0, 0.5,
&seed_);
*/
...
void SelfRobot::PFresample()
{
       ...
       robNo++) // for 4 robots : OMNI4, OMNI3, OMNI1 and OMNI5 in a row
  {
    ...
    // IPP
    /*
        ippsStdDev_32f(&particleSet_[0+robNo*3][0], nParticles_,
       &stdX,ippAlgHintFast);
        ippsStdDev_32f(&particleSet_[1+robNo*3][0], nParticles_,
       &stdY,ippAlgHintFast);
        ippsStdDev_32f(&particleSet_[2+robNo*3][0], nParticles_,
       &stdTheta,ippAlgHintFast);
        */
  }
...
void SelfRobot::selfOdometryCallback(
    const nav_msgs::Odometry::ConstPtr& odometry, int RobotNumber)
{
  ifRobotIsStarted_->at(RobotNumber - 1) = true;
  ...
  for (int i = 0; i < 10; i++)
  {
    if (landmarkfound[i])
    {
      ...
      for (int p = 0; p < nParticles_; p++)
      {
        ...
        //
        //
        // 	  float error = powf( (powf((map_1[i][0] - ObsWorldLM_X),2) +
        // powf((map_1[i][1] - ObsWorldLM_Y),2)) , 0.5 );
        //
        // 	  if(error>1.0)
        // 	  {
        ...
        // "<<map_1[i][0]<<" at Y = "<<map_1[i][1]<<endl<<endl;
        // 	  }
        ...
        Zcap[0] = (map_1[i][0] - particleSet_[0 + (RobotNumber - 1) * 3][p]) *
                      (cos(particleSet_[2 + (RobotNumber - 1) * 3][p])) +
                  (map_1[i][1] - particleSet_[1 + (RobotNumber - 1) * 3][p]) *
                      (sin(particleSet_[2 + (RobotNumber - 1) * 3][p]));

        Zcap[1] = -(map_1[i][0] - particleSet_[0 + (RobotNumber - 1) * 3][p]) *
                      (sin(particleSet_[2 + (RobotNumber - 1) * 3][p])) +
                  (map_1[i][1] - particleSet_[1 + (RobotNumber - 1) * 3][p]) *
                      (cos(particleSet_[2 + (RobotNumber - 1) * 3][p]));
    ...
    const nav_msgs::Odometry::ConstPtr& odometry, int RobotNumber)
{
  ...
  ifRobotIsStarted->at(RobotNumber - 1) = true;
  ...
  for (int i = 0; i < nParticles_; i++)
  {
    ...
    {
      prevParticle =
          Eigen::Rotation2Dd(pfParticlesMate[i][2 + (RobotNumber - 1) * 3])
              .toRotationMatrix();
      prevParticle.translation() =
          Eigen::Vector2d(pfParticlesMate[i][0 + (RobotNumber - 1) * 3],
                          pfParticlesMate[i][1 + (RobotNumber - 1) * 3]);
      curParticle = prevParticle * odom;
      pfParticlesMate[i][0 + (RobotNumber - 1) * 3] =
